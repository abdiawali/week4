...
class ClassList:
    ...
    def __init__(self, max_students):
        ...
        if max_students<=0:
            raise StudentError("Number of students can't be 0 or negative")
        self.class_list = []
    ...
    def is_class_full(self):
        number_of_students=len(self.class_list)
        if number_of_students >= self.max_students:
            return True
        else:
            return False
...
def main():
    ...
    try:
        empty_class= ClassList(-2)
    except:
        print("number of students can't be zero or less")
